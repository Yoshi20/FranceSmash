class TournamentsController < ApplicationController
  ...
  def index
    @tournaments = Tournament.all_fr.active.upcoming.order(date: :asc).includes(:players).paginate(page: params[:page], per_page: Tournament::MAX_PAST_TOURNAMENTS_PER_PAGE)
    @ongoing_tournaments = Tournament.all_fr.active.ongoing.order(date: :asc).includes(:players).paginate(page: params[:page], per_page: Tournament::MAX_PAST_TOURNAMENTS_PER_PAGE)
    @past_tournaments = Tournament.all_fr.active.past.order(date: :desc).includes(:players).paginate(page: params[:page], per_page: Tournament::MAX_PAST_TOURNAMENTS_PER_PAGE)
    if current_user.present? and current_user.super_admin?
      @inactive_tournaments = Tournament.all_fr.where(active: false).order(date: :desc).paginate(page: params[:page], per_page: Tournament::MAX_PAST_TOURNAMENTS_PER_PAGE)
    end
      ...
      respond_to do |format|
        if check_registration_deadline_is_less_than_date(tournament_params) && @tournament.save
          if params[:send_mails]
            Player.all_fr.each do |p|
              if p.user.allows_emails_from_francesmash
      ...
      respond_to do |format|
        ...
        if check_registration_deadline_is_less_than_date(tournament_params) && @tournament.update(tournament_params)
          ...
          if @tournament.weekly? and params[:commit].include?('all')
            ...
            Tournament.all_fr.where('date >= ?', oldDate + 7.days).where("name ILIKE ?", "%#{ActiveRecord::Base.sanitize_sql_like(old_name_without_kw)}%").order(:date).each do |t|
  ...
  def destroy
    if @tournament.active == false
      ...
      if @tournament.weekly? and params[:all]
        ...
        Tournament.all_fr.where('date >= ?', @tournament.date).where(location: @tournament.location).where(host_username: @tournament.host_username).each do |tt|
